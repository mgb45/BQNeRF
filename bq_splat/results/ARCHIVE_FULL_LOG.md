...
> intermediate number, in chronological order. For a short, current-
> conclusions summary, see [`FINDINGS.md`](FINDINGS.md). Section numbers
> (`§N`) referenced anywhere in this codebase's comments/docstrings as
> "`bq_splat/results/FINDINGS.md` section N" refer to the section numbers
> in *this* file (it's the verbatim original `FINDINGS.md`, renamed when
> the short summary was introduced) — `FINDINGS.md` itself is no longer
> numbered by section.
