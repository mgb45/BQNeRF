> **This is the full, unabridged process log** — every bug, every
> intermediate number, in chronological order, including the complete
> multi-step correction history behind a couple of later results. For a
> short, current-conclusions summary organized by topic (real scenes
> first), see [`FINDINGS.md`](FINDINGS.md). Section numbers (`§N`)
> referenced anywhere in this codebase's comments/docstrings as
> "`gs_experiment/results/FINDINGS.md` section N" refer to the section
> numbers in *this* file (it's the verbatim original `FINDINGS.md`,
> renamed when the short summary was introduced) — `FINDINGS.md` itself
> is no longer numbered by section.
